...
describe('database helpers', () => {
  test('should save the user', async () => {
    db.saveUser({ username: 'yuqi', password: 'admin', firstName: 'yuqi', lastName: 'zhu' })
      .then(() => {
        new User({ username: user.username }).fetch().then(found => expect(found).toEqual({ username: 'yuqi' }));
      })
  ...
  test('should save the event', async () => {
    db.saveEvent({
      ...
      new Event({ eventName: 'tennis' }).fetch().then(found => expect(found.attributes.eventName).toEqual('tennis'));
    });
  });

  test('should save the sport', async () => {
    db.saveSport({ sportName: 'tennis' }).then(() => {
      new Sport({ sportName: 'tennis' }).fetch().then(found => expect(found && 1).toBe(1));
    });
  });
  ...
  test('should save the field', async () => {
    db.saveField({ fieldName: 'tennis court' }).then(() => {
      new Field({ fieldName: 'tennis court' }).fetch().then(found => expect(found && 1).toBe(1));
    });
  });

  test('should save the venue', async () => {
    db.saveVenue({ venueName: 'central park' }).then(() => {
      new Venue({ venueName: 'central park' }).fetch().then(found => expect(found && 1).toBe(1));
    });
  });

  test('should save the message', async () => {
    db.saveMessage({ username: 'yuqi', eventId: 1, body: 'Hi' }).then(() => {
      new Message({ username: 'yuqi' }).fetch().then(found => expect(found && 1).toBe(1));
    });
  });
